...
def collate_LLM(batch, tokenizer, modality, is_trainval=True):
    ...
    if is_trainval and isinstance(batch, dict):
        batch = [batch]
    elif not is_trainval and isinstance(batch, list):
        if len(batch) != 1:
            raise ValueError("Inference collate expects a batch size of 1.")
        batch = batch[0]
    
    if is_trainval:
        tokens = []
    if modality in ["audio", "audiovisual", "audiovisual_avhubert"]:
        audios = [] if is_trainval else batch["audio"]
    else:
        audios = None
    if modality in ["video", "audiovisual", "audiovisual_avhubert"]:
    ...
    if audios is not None:
        audio_list = audios if is_trainval else [audios]
        audio_stack = torch.nn.utils.rnn.pad_sequence(audio_list, batch_first=True, padding_value=0)
        batch_out["audio"] = audio_stack if is_trainval else audio_stack.unsqueeze(0)
        batch_out["lengths"] = torch.tensor(lengths)
        
    if videos is not None:
        video_list = videos if is_trainval else [videos]
        video_stack = torch.nn.utils.rnn.pad_sequence(video_list, batch_first=True, padding_value=0)
...
class DataModule_LLM(LightningDataModule):
    def __init__(
        ...
        self.dataset_name = getattr(args, "dataset_name", "lrs3").lower()
        self.grid_max_train_samples = getattr(args, "grid_max_train_samples", None)
        self.grid_max_val_samples = getattr(args, "grid_max_val_samples", None)
        self.grid_max_test_samples = getattr(args, "grid_max_test_samples", None)
        split_ratio_arg = getattr(args, "grid_split_ratio", (0.8, 0.1, 0.1))
        if split_ratio_arg is None:
            self.grid_split_ratio = (0.8, 0.1, 0.1)
        else:
            if len(split_ratio_arg) != 3:
                raise ValueError("grid_split_ratio must contain exactly three values.")
            total = float(sum(split_ratio_arg))
            if total == 0:
                raise ValueError("grid_split_ratio values must sum to a positive number.")
            self.grid_split_ratio = tuple(float(x) / total for x in split_ratio_arg)
    ...
    def train_dataloader(self):
        ...
        if self.dataset_name == "grid":
            video_transform = VideoTransform("train") if self.args.modality in ["video", "audiovisual", "audiovisual_avhubert"] else None
            audio_transform = AudioTransform("train", is_avhubert_audio=self.is_avhubert_audio) if self.args.modality in ["audio", "audiovisual", "audiovisual_avhubert"] else None
            dataset = GRIDDataset(
                root_dir=self.args.root_dir,
                modality=self.args.modality,
                split="train",
                split_ratio=self.grid_split_ratio,
                video_transform=video_transform,
                audio_transform=audio_transform,
                max_samples=self.grid_max_train_samples,
                shuffle=True,
                seed=self.args.seed,
            )
        ...
        dataset = AVDataset_LLM(
            ...
            subset="train",
            modality=self.args.modality,
            audio_transform=AudioTransform("train", is_avhubert_audio = self.is_avhubert_audio),
    ...
    def val_dataloader(self):
        if self.dataset_name == "grid":
            video_transform = VideoTransform("val") if self.args.modality in ["video", "audiovisual", "audiovisual_avhubert"] else None
            audio_transform = AudioTransform("val", is_avhubert_audio=self.is_avhubert_audio) if self.args.modality in ["audio", "audiovisual", "audiovisual_avhubert"] else None
            dataset = GRIDDataset(
                root_dir=self.args.root_dir,
                modality=self.args.modality,
                split="val",
                ...
                video_transform=video_transform,
                audio_transform=audio_transform,
                max_samples=self.grid_max_val_samples,
                seed=self.args.seed,
            )
            dataloader = torch.utils.data.DataLoader(
                dataset,
        ...
        dataset = AVDataset_LLM(
            ...
            subset="val",
            modality=self.args.modality,
            audio_transform=AudioTransform("val", is_avhubert_audio = self.is_avhubert_audio),
    ...
    def test_dataloader(self):
        if self.dataset_name == "grid":
            video_transform = VideoTransform("test") if self.args.modality in ["video", "audiovisual", "audiovisual_avhubert"] else None
            audio_transform = AudioTransform(
                "test",
                snr_target=self.args.decode_snr_target,
                is_avhubert_audio=self.is_avhubert_audio,
            ) if self.args.modality in ["audio", "audiovisual", "audiovisual_avhubert"] else None
            dataset = GRIDDataset(
                root_dir=self.args.root_dir,
                modality=self.args.modality,
                split="test",
                ...
                video_transform=video_transform,
                audio_transform=audio_transform,
                max_samples=self.grid_max_test_samples,
                seed=self.args.seed,
            )
            dataloader = torch.utils.data.DataLoader(
                dataset,
